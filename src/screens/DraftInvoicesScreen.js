// FILE: src/screens/DraftInvoicesScreen.js
...
const DraftInvoiceItem = ({ item, isExpanded, onExpand, onEdit, onDelete }) => {
        ...
        <View style={[styles.itemWrapper, { borderLeftColor: statusStyle.borderColor }]}>
            <TouchableOpacity style={styles.itemContainer} onPress={onExpand}>
                <View style={styles.infoContainer}>
                     <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <Text style={styles.itemTitle}>{item.pirkejas || 'Nenurodytas Pirkėjas'}</Text>
            ...
            {isExpanded && (
                <View style={styles.expandedContent}>
                    ...
                    <View style={styles.buttonRow}><TouchableOpacity style={[globalStyles.button, styles.actionBtn, {backgroundColor: colors.danger}]} onPress={onDelete}><Text style={globalStyles.buttonText}>Ištrinti</Text></TouchableOpacity><TouchableOpacity style={[globalStyles.button, styles.actionBtn, {backgroundColor: colors.primary}]} onPress={onEdit}><Text style={globalStyles.buttonText}>Redaguoti</Text></TouchableOpacity></View>
                </View>
            )}
        </View>
    );
...
export default function DraftInvoicesScreen({ navigation }) {
    const { draftSaskaitos, handleUpdateDraftInvoice, handleDeleteDraftInvoice } = useContext(InvoicesContext);
    ...
    const handleGeneratePdf = (invoice) => { if (!invoice) return; showCustomAlert( 'PDF Generavimas', `Funkcija ruošiama. Reikalinga speciali biblioteka, kad būtų galima sukurti PDF failą pagal sąskaitos ${invoice.saskaitosNr} duomenis.` ); };
    
    const filteredDraftSaskaitos = useMemo(() => {
        return draftSaskaitos.filter(invoice => {
            if (!invoice || typeof invoice.busena === 'undefined' || typeof invoice.pirkejas === 'undefined') return false;
            const statusMatch = !filters.status || invoice.busena === filters.status;
            const entityMatch = !filters.entity || invoice.pirkejas === filters.entity;
            let dateMatch = true;
    ...
    }, [draftSaskaitos, filters]);
    const statusFilterOptions = settings.busenaOptions.filter(b => b !== 'Nuosavos lėšos');

    return (
        <View style={globalStyles.screenContent}>
             <TouchableOpacity style={styles.filterToggleButton} onPress={() => { LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut); setShowFilters(!showFilters); }}>
                <Text style={styles.filterToggleButtonText}>Filtruoti {showFilters ? '▲' : '▼'}</Text>
            </TouchableOpacity>
            ...
            {showFilters && (
                <FilterPanel
                    ...
                    entityOptions={settings.pirkejaiOptions}
                    entityLabel="Pirkėjas"
                    statusOptions={statusFilterOptions}
                    statusStyleGetter={getStatusStyle}
                />
            )}
            
            <FlatList
                data={filteredDraftSaskaitos}
                renderItem={({ item }) => ( <DraftInvoiceItem item={item} isExpanded={expandedItemId === item.id} onExpand={() => toggleExpand(item.id)} onEdit={() => setEditingInvoice(item)} onDelete={() => handleDeleteDraftInvoice(item.id)} /> )}
            ...
            />
            {editingInvoice && ( <EditInvoiceModal visible={!!editingInvoice} onClose={() => setEditingInvoice(null)} invoice={editingInvoice} onSave={(id, data) => { handleUpdateDraftInvoice(id, data); setEditingInvoice(null); }} onGeneratePdf={() => handleGeneratePdf(editingInvoice)} pirkejaiOptions={settings.pirkejaiOptions} busenaOptions={settings.busenaOptions} mokejimoPaskirtisOptions={settings.mokejimoPaskirtisOptions} isDraft={true} /> )}
        </View>
    );
}